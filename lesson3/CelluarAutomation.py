...
pattern = { 1:'...', 2:'..x', 4:'.x.', 8:'.xx',
            16:'x..', 32:'x.x', 64:'xx.', 128:'xxx' }

def pattern_numbers(p_num):
    res = []
    for n in range(7, -1, -1):
        if p_num / (2**n) > 0:
            p_num -= (2**n); res.append(2**n)
    return res
    
def cellular_automaton_bad(string, p, generation):
    numbers = pattern_numbers(p) 
    target, result = '', '';
    sources = [pattern[n] for n in numbers]
    #print sources
    for _ in range(0, generation):
        result = ''
        for n in range(len(string)):
            if n == 0: target = string[-1] + string[:2]
            elif n == len(string)-1: target = string[-2:] + string[0]
            else: target = string[n-1:n+2]
            #print n, target
