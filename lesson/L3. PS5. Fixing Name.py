...
def fix_name(name):
    ...
    return get_namelist(name)
    
def get_namelist(string):    
    names, entry, is_new = [], '', False
    for c in string[1:]:
        if c == '|': is_new = True; continue
        elif c == '}': names.append(entry); break;
        else:
            if is_new is True: names.append(entry); entry = ''; is_new = False
            entry += c
