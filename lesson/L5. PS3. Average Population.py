...
def make_pipeline():
    ...
    pipeline = [  { "$unwind": "$isPartOf" },
                  { "$match": { "country_label": { "$ne": "NULL" } } },
                  { "$group": { "_id": { "country_label":"$country_label", "region":"$isPartOf" }, "avgRegion": { "$avg": "$populationTotal" } } },
                  { "$group": { "_id": "$_id.country_label", "avgRegionalPopulation": { "$avg": "$avgRegion" } } },
