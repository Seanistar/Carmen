...
def in_query():
    ...
    #query = { "manufacturer_label":{"$regex":"[Ff]ord"}, "assembly_label":{"$in":["Germany","United Kingdom","Japan"]} }
    query = { "manufacturer_label":{"$regex":"[Ff]ord"}, \
            "assembly_label":{"$in": [re.compile(".*Germany.*"),re.compile(".*United Kingdom.*"), re.compile(".*Japan.*")] } }
